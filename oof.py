...
async def source(ctx):
	...
	log('Sent source code to {0.author} at {0.channel} ({0.server})'.format(ctx.message))
...
async def help(ctx):
	...
	try:	em_1.color = ctx.message.server.me.color or discord.Embed.Empty
	except:	em_1.color = discord.Embed.Empty
	...
	try:	em_2.color = ctx.message.server.me.color or discord.Embed.Empty
	except:	em_2.color = discord.Embed.Empty
...
try:	bot.run(token)
except:	log('Could not run the bot')